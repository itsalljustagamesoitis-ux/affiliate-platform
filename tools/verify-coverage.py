...
# Minimum products required per article type.
# comparison and review generate article_specific_pros dynamically — default_pros not required.
# roundup and buyer_guide render ProductCard with default_pros — pros gate applies.
_TYPE_MIN_PRODUCTS = {
    "comparison":  2,
    "review":      1,
    "roundup":     4,
...
_PROS_REQUIRED_TYPES = {"roundup", "buyer_guide"}


def count_usable_products(article: dict, products: dict) -> int:
    """Count products assigned to this article that exist in catalog.

    For roundup/buyer_guide types, also requires non-empty default_pros (ProductCard
    needs them). For comparison/review, any catalog-present product is usable since
    article_specific_pros are generated by the producer.
    """
    article_type = article.get("type", "roundup")
    require_pros = article_type in _PROS_REQUIRED_TYPES
    ...
    for key in assigned:
        ...
        if prod and isinstance(prod, dict):
            if require_pros:
                ...
                count += 1  # catalog existence is sufficient for comparison/review
    return count


def _min_for_article(article: dict, global_min: int) -> int:
    """Return the effective minimum product count for this article type."""
    article_type = article.get("type", "")
    return _TYPE_MIN_PRODUCTS.get(article_type, global_min)
...
def run_check(args) -> int:
    ...
    for article in live_articles:
        ...
        min_products = _min_for_article(article, global_min)
    ...
    print(f"Min products required: {global_min} (type-aware: comparison=2, review=1, roundup=4)")
    ...
    print(f"  OK (type-min met):  {len(ok_articles)} articles ({ok_pct}%)")
    print(f"  Shortfall:          {len(shortfall_articles)} articles ({sf_pct}%)")
