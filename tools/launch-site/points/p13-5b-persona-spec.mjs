/**
 * Point 13.5b — Persona-spec compliance validator
 * Bucket A: LLM-based validator (~$0.0011/article); HARD failures regenerate
 */

import { existsSync } from 'fs'
import { validatorLoop } from '../escalation.mjs'

export const POINT_ID = '13.5b'
export const BUCKET = 'A'

export async function run(ctx) {
  const { slug, siteDir, platformDir, dryRun, log, execTool } = ctx

  const validator = `${platformDir}/scripts/validate-persona-spec-compliance.mjs`
  if (!existsSync(validator)) {
    return { status: 'fail', message: `validate-persona-spec-compliance.mjs not found` }
  }

  if (dryRun) {
    log.info('(dry-run) would run validate-persona-spec-compliance.mjs (~$0.0011/article via Haiku)')
    return { status: 'pass', logFields: { dry_run: true } }
  }

  const result = await validatorLoop(
    () => {
      const r = execTool(`node "${validator}" --site ${slug} --json`, { timeout: 600000 })
      let parsed
    ...
    { label: 'validate-persona-spec-compliance', log },
  )

  if (result.status === 'skip_listed') {
    log.warn(`Persona-spec validator: ${result.skipIds.length} articles moved to skip-list`)
